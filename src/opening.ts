 ...
 *       黒地に素の戻るボタンだけが浮く状態を避け、「‹ 読み進める」と「もどる ›」を中央に提示する。
 ...
 * 依存: state.ts（「もどる」の遷移先 URL 解決のみ）。読書点比率は CSS 変数 --reading-anchor を読むのみ（bg.ts と同じ源）
 *
...
// 「読み進める」「もどる」ボタンにイベントを登録する。main.ts が起動時に1度だけ呼ぶ。
// init(): void
export function init(): void {
    ...
    // 「もどる」：本文端の戻るボタン（nav.ts #btn-prev）と同じ遷移先（前 sec／当 ep 先頭は当 ep タイトル）。
