 ...
 * 依存: なし（他モジュールを import しない＝疎結合。読書点は CSS 変数 --reading-anchor を読むのみ。
 *       現在シーン・既読・進捗・オートセーブの利用は通知を受けた reader.ts 側で行う）
 *
 * クロスフェード（台形プラトー方式・要件 06-3）:
 *   - 各シーンコンテナ（#scene-content .scene・DOM 順＝読書順）の getBoundingClientRect() 中心 c_k を読む
 *   - 読書点 anchorX（--reading-anchor 比率 × #main-container 幅）が中心列のどこに落ちるかを線形補間し
 *     連続値 P を得る（現在シーン導出用。anchorX === c_k のとき P = k）
 ...
 *   REVERSE_DIRECTION（vertical-rl の方向反転・実機確認用）
 */
...
const REVERSE_DIRECTION = false;  // vertical-rl RTL は false（中心列は x 降順）。実機で逆なら true
...
// init(layers: BgLayerSpec[], ep: number): void
export function init(layers: BgLayerSpec[], ep: number): void {
  _specs = layers;
  _ep = ep;
  _loaded = new Set();
...
function _emit(container: HTMLElement): void {
  ...
  if (n > 0) {
    ...
    const anchorX = containerRect.left + _readAnchorRatio() * containerRect.width;
    ...
    // 先頭の黒い恒久余白：本文の読み始め辺（RTL は右端 textRight）を仮想境界に渡し、開幕の接近区間で黒→先頭背景へ
    // フェードアップし、読書点が読み始め辺に到達した時点で全表示にする（移動完了＝全表示）。
    const leadEdge = REVERSE_DIRECTION ? textLeft : textRight;
    // フェード窓＝接近距離（読書点〜画面の読み始め端＝開幕余白を詰める距離）。CROSSFADE_PX を上限に収めることで、
    // 静止した開幕（scrollLeft 0）では必ず黒、狭い画面でも開幕に背景が漏れない。
    const approach = REVERSE_DIRECTION ? (containerRect.right - anchorX) : (anchorX - containerRect.left);
    const leadWindow = Math.min(CROSSFADE_PX, Math.max(0, approach));
    const opacities = layerOpacities(centers, anchorX, CROSSFADE_PX, REVERSE_DIRECTION, boundaries, leadEdge, leadWindow);
    for (let k = 0; k < _layers.length; k++) {
      _layers[k].style.opacity = String(k < opacities.length ? opacities[k] : 0);
    }
    currentScene = deriveCurrentScene(computeP(centers, anchorX, REVERSE_DIRECTION), n);
    // 進捗は本文領域（全シーンの外接区間 [textLeft, textRight]）を読書点が走る割合。前後の空白余白では 0/1 に固定。
    progress = computeProgress(textLeft, textRight, anchorX, REVERSE_DIRECTION);
...
function _setSrc(k: number): void {
  ...
  _layers[k].src = `${import.meta.env.BASE_URL}ep${_pad(_ep)}/img/${spec.bgFile}`;
}
