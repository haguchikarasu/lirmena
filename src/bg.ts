...
import type { ScrollNotification, BgLayerSpec, BgSource } from './types';
...
// init(layers: BgLayerSpec[], source: BgSource): void
export function init(layers: BgLayerSpec[], source: BgSource): void {
  _specs = layers;
  _source = source;
  _loaded = new Set();
...
function _setSrc(k: number): void {
  ...
  _layers[k].src = `${import.meta.env.BASE_URL}${buildBgUrl(_source, spec.bgFile)}`;
}

// 背景画像 URL のパス組み立て（BASE_URL 抜き＝呼び出し側で連結する）。純関数・DOM 非依存。
//   source.kind === 'ep'        → vol[XX]/ep[YY]/img/{ファイル名}（本文モード）
//   source.kind === 'afterword' → vol[XX]/{ファイル名}（あとがきモード＝vol 直下）
// buildBgUrl(source: BgSource, file: string): string
export function buildBgUrl(source: BgSource, file: string): string {
  const volPart = `vol${_pad(source.vol)}`;
  if (source.kind === 'afterword') {
    return `${volPart}/${file}`;
  }
  return `${volPart}/ep${_pad(source.ep)}/img/${file}`;
}
